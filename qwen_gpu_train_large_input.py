...
from transformers import AutoModelForCausalLM, AutoTokenizer, Trainer, TrainingArguments,PreTrainedModel, Qwen3Config,Qwen3ForCausalLM


class ContinuousRewardModel(PreTrainedModel):
...
def prepare_dataset(model_name_or_path: str, data_path: str):
    ...
    #         padded_batch["labels"] = torch.tensor(labels, dtype=torch.float)
...
def train_continuous_reward_model():
    # 1. 配置参数
    model_name_or_path = "/mlx_devbox/users/wukunqiong/playground/arnold_workspace_root/Code/Qwen_ar_score/Qwen3-1.7B"  # 基础模型
    data_path = "/mlx_devbox/users/wukunqiong/playground/arnold_workspace_root/Code/Qwen_ar_score/train/1031-thinking-优势分-训练1.csv"  # 连续分数数据
      # 模型保存路径
    output_dir = "./continuous_reward_model-1104"
    ...
    pretrained_state = Qwen3ForCausalLM.from_pretrained(
        model_name_or_path,
        # dtype=torch.float  # 改用bfloat16，比fp16更稳定
    ).state_dict()
    ...
    training_args = TrainingArguments(
        ...
        learning_rate=2e-5,  # 稍微提高学习率以补偿更大的累积步数
        num_train_epochs=1,
        ...
        dataloader_num_workers=2,  # 使用多进程加载数据
        dataloader_pin_memory=False,  # 加速数据传输
    )
